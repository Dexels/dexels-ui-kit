...
export const StyledFormElementLabel = styled.div<StyledFormElementLabelProps>`
    ...
    ${({ hasTextIndentation, isActive, variant }): SimpleInterpolation =>
        variant === InputVariant.COMPACT &&
        css`
            top: ${isActive ? '-16px' : 0};
            left: ${hasTextIndentation ? '10px' : 0};
        `}

    ${({ backgroundColor, hasTextIndentation, isActive, theme, variant }): SimpleInterpolation =>
        variant === InputVariant.OUTLINE &&
        css`
            top: ${isActive ? '-8px' : '50%'};
            left: ${hasTextIndentation ? theme.spacing(2.5) : theme.spacing(1.5)};
